...
// ─── ROLE CONFIG ──────────────────────────────────────────────────────────────
const ROLE_MAP = {
  ADMIN: { label: "Owner · Admin", color: "text-violet-400", bg: "bg-violet-500/10 border-violet-500/30" },
  hr_manager: { label: "HR Manager", color: "text-indigo-400", bg: "bg-indigo-500/10 border-indigo-500/30" },
  hr_executive: { label: "HR Executive", color: "text-blue-400", bg: "bg-blue-500/10 border-blue-500/30" },
  team_lead: { label: "Team Lead", color: "text-cyan-400", bg: "bg-cyan-500/10 border-cyan-500/30" },
  manager: { label: "Manager", color: "text-teal-400", bg: "bg-teal-500/10 border-teal-500/30" },
  employee: { label: "Employee", color: "text-slate-400", bg: "bg-slate-500/10 border-slate-500/30" },
};

const STATUS_MAP = {
  active: { label: "Active", dot: "bg-emerald-400", pill: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" },
  pending_offer: { label: "Offer Pending", dot: "bg-amber-400 animate-pulse", pill: "bg-amber-500/10 text-amber-400 border-amber-500/20" },
  rejected: { label: "Declined", dot: "bg-rose-400", pill: "bg-rose-500/10 text-rose-400 border-rose-500/20" },
};
...
function StatusBadge({ status }) {
  const s = STATUS_MAP[status] || { label: status, dot: "bg-slate-400", pill: "bg-slate-500/10 text-slate-400 border-slate-500/20" };
  return (
...
function StatCard({ label, value, sub, icon, accent = "indigo" }) {
  ...
  return (
    <div className="relative bg-[#1a1e2a] border border-[#252d3d] rounded-2xl p-5 overflow-hidden group hover:border-indigo-500/30 transition-all duration-300">
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${colors[accent]} border flex items-center justify-center text-sm`}>
            {icon}
          </div>
        </div>
  ...
  return (
    <div className="flex h-screen bg-[#0f1117] overflow-hidden">
      ...
      <aside className={`
        ...
        bg-[#13161e] border-r border-[#252d3d]
      ...
      `}>
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-[#252d3d]">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold text-white truncate">{company?.name || "Workspace"}</p>
            <p className="text-[10px] text-indigo-400 font-semibold uppercase tracking-wider">HRMS Portal</p>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
          <p className="px-3 mb-2 text-[9px] font-bold text-slate-600 uppercase tracking-widest">Navigation</p>
              ...
              <button key={item.key}
                ...
                className={`
                  ...
                  ${active
                    ...
                    : "text-slate-400 hover:text-slate-200 hover:bg-[#1e2334]"
                  }
                `}
              >
                ...
                {item.key === "employees" && (
                  <span className={`ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-md ${active ? "bg-white/20 text-white" : "bg-[#252d3d] text-slate-400"}`}>
                    {employees.length}
                  </span>
                )}
          ...
          {isHR && (
            <>
              <div className="pt-4 pb-1">
                <p className="px-3 mb-2 text-[9px] font-bold text-slate-600 uppercase tracking-widest">Actions</p>
              </div>
              <button
                onClick={() => { setIsInviteModalOpen(true); setSidebarOpen(false); }}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-indigo-300 hover:bg-indigo-500/10 hover:text-indigo-200 border border-indigo-500/20 transition-all"
              >
        ...
        {/* Footer */}
        <div className="px-4 pb-5 pt-3 border-t border-[#252d3d] space-y-3">
          {/* Realtime Status */}
          <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-[#0f1117] border border-[#252d3d]">
            <div className="flex items-center gap-2 text-[10px] text-slate-500">
              <span className={`w-1.5 h-1.5 rounded-full ${realtimeStatus === "active" || realtimeStatus === "synced" ? "bg-emerald-400 animate-pulse" : "bg-amber-400"}`} />
              <span>Realtime</span>
            </div>
            <span className={`text-[10px] font-bold font-mono ${realtimeStatus === "active" || realtimeStatus === "synced" ? "text-emerald-400" : "text-amber-400"}`}>
              {realtimeStatus}
            </span>
          </div>

          <button onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-semibold text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 border border-[#252d3d] hover:border-rose-500/20 transition-all">
            <span>→</span>
            <span>Sign Out</span>
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Top Bar */}
        <header className="h-14 shrink-0 flex items-center justify-between px-5 border-b border-[#252d3d] bg-[#13161e]/80 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="md:hidden p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-[#1e2334]">
              ☰
            </button>
            <div>
          ...
          <div className="flex items-center gap-3">
            {isHR && (
              <button onClick={() => setIsInviteModalOpen(true)}
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold transition shadow-lg shadow-indigo-500/20">
                ＋ Invite
              </button>
            )}
        ...
        <main className="flex-1 overflow-y-auto p-5 lg:p-7 space-y-6 animate-fadeIn">
              ...
              {isAdmin ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    {/* Profile Card */}
                    <div className="lg:col-span-2 bg-[#1a1e2a] border border-[#252d3d] rounded-2xl p-6 space-y-5">
                      <h3 className="text-sm font-bold text-white border-b border-[#252d3d] pb-4">My Profile Details</h3>
                        ...
                        {[
                          ...
                          <div key={label} className="space-y-1">
                            ...
                            {badge
                              ...
                              : <p className={`text-sm font-semibold ${mono ? "text-indigo-400 font-mono" : "text-slate-200"}`}>{value}</p>
                            }
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="bg-[#1a1e2a] border border-[#252d3d] rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div>
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white text-xs font-bold shadow-lg shadow-indigo-600/20 transition-all shrink-0 cursor-pointer"
                    >
          ...
          {/* ── TAB: TEAM DIRECTORY ──────────────────────────────────────── */}
          {activeTab === "employees" && (
            <div className="bg-[#1a1e2a] border border-[#252d3d] rounded-2xl overflow-hidden">
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-5 border-b border-[#252d3d]">
                <div>
                  <h3 className="text-sm font-bold text-white flex items-center gap-2">
                    Team Directory
                    <span className="px-2 py-0.5 rounded-md bg-[#252d3d] text-slate-400 text-[10px] font-mono">
                      {filtered.length}/{employees.length}
                    </span>
                  </h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {isHR ? "Manage employee accounts and credential status" : "Browse colleagues across departments"}
                  </p>
                </div>
                {isHR && (
                  <button onClick={() => setIsInviteModalOpen(true)}
                    className="self-start sm:self-auto flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold transition">
                    ＋ Invite Member
                  </button>
                )}
              </div>
              ...
              {/* Filters */}
              <div className="flex flex-col sm:flex-row items-center gap-3 px-6 py-3 bg-[#0f1117]/40 border-b border-[#252d3d]">
                <div className="relative flex-1 w-full sm:max-w-64">
                  ...
                  <input type="text" placeholder="Search name, email, department…" value={searchQuery}
                    ...
                    className="w-full bg-[#1a1e2a] border border-[#252d3d] rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500 transition" />
                </div>
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
                  className="bg-[#1a1e2a] border border-[#252d3d] text-slate-300 text-xs rounded-lg px-3 py-1.5 focus:outline-none focus:border-indigo-500 w-full sm:w-auto">
              ...
              {loadingEmployees ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody>
                      {filtered.map((emp) => (
                        <tr key={emp.id} className="border-b border-[#252d3d]/60 hover:bg-[#1e2334] transition-colors group">
                              ...
                              <div>
                                <div className="font-semibold text-slate-200 flex items-center gap-1.5">
                                  ...
                                  {emp.auth_user_id && onlineUserIds.has(emp.auth_user_id) && (
                                    <span className="inline-flex items-center gap-0.5 px-1 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[9px] font-bold">
                          ...
                          <td className="py-3.5 px-4">
                            <p className="font-medium text-slate-300">{emp.department || "General"}</p>
                            <p className="text-slate-500 text-[10px]">{emp.designation || "—"}</p>
                          </td>
                          <td className="py-3.5 px-4 font-mono text-indigo-400 font-semibold">
                            {emp.username ? `@${emp.username}` : <span className="text-slate-600 font-sans italic">Pending</span>}
                          </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="lg:col-span-2 bg-[#1a1e2a] border border-[#252d3d] rounded-2xl p-6 space-y-5">
                <div className="flex items-center justify-between border-b border-[#252d3d] pb-4">
                  <div>
                    <h3 className="text-sm font-bold text-white">Department Records</h3>
                    ...
                    {dbDepartments.map((dept) => {
                      ...
                      return (
                        <div key={dept.id || dept.name}
                          className="p-4 rounded-xl bg-[#0f1117] border border-[#252d3d] hover:border-indigo-500/30 transition-colors space-y-3">
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-bold text-slate-200">{dept.name}</p>
                            {dept.code && (
                              <span className="px-2 py-0.5 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 font-mono text-[10px] font-semibold">
                                {dept.code}
                              </span>
                            )}
                          </div>
                          {dept.description && <p className="text-xs text-slate-500 leading-relaxed">{dept.description}</p>}
                          <div className="flex items-center justify-between pt-2 border-t border-[#252d3d] text-[10px]">
          ...
          {activeTab === "settings" && (
            <div className="max-w-3xl space-y-6">
              <form onSubmit={handleProfileSave} className="bg-[#1a1e2a] border border-[#252d3d] rounded-2xl overflow-hidden shadow-xl">
                {/* Profile Header & Photo Upload */}
                <div className="bg-gradient-to-r from-indigo-600/20 via-violet-600/15 to-purple-600/10 border-b border-[#252d3d] px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-5">
                  <div className="flex items-center gap-4">
                    <div className="relative group">
                      {profileForm.avatarUrl ? (
                        ...
                        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-extrabold text-3xl shadow-xl shadow-indigo-500/20">
                          {avatar}
                        </div>
                      )}
                      <label className="absolute inset-0 rounded-2xl bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer text-white text-xs font-bold gap-1">
                        📷 Change
                        <input
                          type="file"
                          accept="image/*"
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">{employeeProfile?.email || userSession?.email}</p>
                  ...
                  <label className="cursor-pointer px-4 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 border border-indigo-500/30 text-xs font-semibold transition flex items-center gap-2">
                    📷 Upload Photo
                    <input
                      type="file"
                      accept="image/*"
                ...
                {/* Notifications */}
                {profileMsg.error && (
                  <div className="mx-6 mt-6 p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs font-medium flex items-center gap-2">
                    <span>⚠️</span>
                    <span>{profileMsg.error}</span>
                  </div>
                )}
                {profileMsg.success && (
                  <div className="mx-6 mt-6 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium flex items-center gap-2">
                    <span>🎉</span>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="space-y-3">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-[#252d3d] pb-2 flex items-center justify-between">
                      <span>Official Workspace Records</span>
                      <span className="text-[10px] text-amber-400 normal-case font-medium flex items-center gap-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 flex items-center justify-between">
                          ...
                          <span className="text-amber-400 font-semibold normal-case">🔒 Not Editable</span>
                        </label>
                        <input
                          type="text"
                          disabled
                          ...
                          className="w-full px-3.5 py-2.5 rounded-xl bg-[#0f1117]/80 border border-[#252d3d] text-amber-400 font-mono font-bold text-xs cursor-not-allowed select-none outline-none shadow-inner"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2.5 rounded-xl bg-[#0f1117]/80 border border-[#252d3d] text-slate-400 font-mono text-xs cursor-not-allowed select-none outline-none"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2.5 rounded-xl bg-[#0f1117]/80 border border-[#252d3d] text-slate-400 text-xs cursor-not-allowed select-none outline-none"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2.5 rounded-xl bg-[#0f1117]/80 border border-[#252d3d] text-indigo-400 font-mono text-xs cursor-not-allowed select-none outline-none"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 pt-2">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-[#252d3d] pb-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2.5 rounded-xl bg-[#0f1117] border border-[#252d3d] text-slate-100 placeholder-slate-600 text-xs focus:border-indigo-500 outline-none transition"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-3.5 py-2.5 rounded-xl bg-[#0f1117] border border-[#252d3d] text-slate-100 placeholder-slate-600 text-xs focus:border-indigo-500 outline-none transition resize-none"
                      />
                    </div>
                  </div>

                  {/* Actions */}
                  <div className="pt-4 border-t border-[#252d3d] flex flex-col sm:flex-row items-center justify-between gap-4">
                    <button
                      type="button"
                      onClick={handleLogout}
                      className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-rose-500/20 bg-rose-500/10 text-rose-400 text-xs font-bold hover:bg-rose-500/20 transition w-full sm:w-auto justify-center"
                    >
                      🚪 Sign Out
                    </button>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold disabled:opacity-50 transition shadow-lg shadow-indigo-500/25 w-full sm:w-auto justify-center"
                    >
                      {isSavingProfile ? (
                        <>
      ...
      {/* ── INVITE MODAL ─────────────────────────────────────────────────────── */}
      {isInviteModalOpen && isHR && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
          <div className="bg-[#1a1e2a] border border-[#252d3d] rounded-2xl w-full max-w-md shadow-2xl shadow-black/60">
            {/* Modal Header */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#252d3d]">
              <div>
                <h3 className="text-sm font-bold text-white">Invite Employee</h3>
                <p className="text-xs text-slate-500 mt-0.5">Send invitation & auto-generate credentials</p>
              </div>
              <button onClick={closeInviteModal}
                className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#252d3d] text-slate-400 hover:text-white text-sm transition">
                ✕
              </button>
            </div>
                ...
                <div className="space-y-4">
                  {inviteSuccessData.emailSent ? (
                    <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-center space-y-1">
                      <p className="text-sm font-bold text-emerald-400">🎉 Invitation Sent!</p>
                      <p className="text-xs text-slate-400">Email delivered to <strong className="text-white font-mono">{inviteSuccessData.employee?.email}</strong></p>
                    </div>
                  ) : (
                    <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-300 space-y-1">
                      ...
                      <p className="text-slate-400">{inviteSuccessData.emailErrorMessage}</p>
                    </div>
                  )}
                  <div className="bg-[#0f1117] rounded-xl border border-[#252d3d] p-4 space-y-3 text-xs">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider pb-2 border-b border-[#252d3d]">Offer Links</p>
                    <div>
                      <p className="text-emerald-400 font-semibold mb-1">✓ Accept Offer:</p>
                      <div className="bg-[#1a1e2a] rounded-lg p-2 font-mono text-slate-300 text-[10px] break-all select-all">{inviteSuccessData.inviteUrls?.acceptUrl}</div>
                    </div>
                    <div>
                      <p className="text-rose-400 font-semibold mb-1">✕ Decline Offer:</p>
                      <div className="bg-[#1a1e2a] rounded-lg p-2 font-mono text-slate-300 text-[10px] break-all select-all">{inviteSuccessData.inviteUrls?.declineUrl}</div>
                    </div>
                  </div>
                  <button onClick={closeInviteModal}
                ...
                <form onSubmit={handleInviteSubmit} className="space-y-4">
                  {[
                    ...
                    <div key={f.name}>
                      ...
                      <input type={f.type} name={f.name} placeholder={f.placeholder} required={f.required}
                        ...
                        className="w-full px-3 py-2.5 rounded-xl bg-[#0f1117] border border-[#252d3d] text-slate-200 placeholder-slate-600 text-xs focus:border-indigo-500 outline-none transition" />
                    </div>
                  ))}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select name="role" value={inviteForm.role} onChange={(e) => setInviteForm({ ...inviteForm, role: e.target.value })}
                        className="w-full px-3 py-2.5 rounded-xl bg-[#0f1117] border border-[#252d3d] text-slate-200 text-xs focus:border-indigo-500 outline-none transition">
                  ...
                  <div className="flex gap-3 pt-2">
                    <button type="button" onClick={closeInviteModal}
                      className="flex-1 py-2.5 rounded-xl bg-[#252d3d] hover:bg-[#2d3550] text-slate-300 text-xs font-semibold transition">
                      Cancel
                    </button>
                    <button type="submit" disabled={isSubmittingInvite}
                      className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold disabled:opacity-50 flex items-center justify-center gap-2 transition shadow-lg shadow-indigo-500/20">
                      {isSubmittingInvite
