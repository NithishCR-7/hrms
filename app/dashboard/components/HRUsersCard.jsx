    ...
    <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 sm:p-7 space-y-5 hover:border-slate-700/80 transition duration-300 shadow-xl flex flex-col justify-between">
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800/80 pb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white tracking-tight">HR Management Team</h3>
              <p className="text-xs text-slate-400">Realtime HR team & pending invitations</p>
            </div>
          </div>
        ...
        {totalHRHeadcount === 0 ? (
          <div className="py-6 text-center space-y-3 bg-slate-950/40 rounded-2xl p-4 border border-slate-800/60">
            <p className="text-xs text-slate-400">No dedicated HR Manager or HR Executive added yet.</p>
          ...
          <div className="space-y-2.5 max-h-52 overflow-y-auto pr-1">
            ...
            {activeHR.map((hr) => (
              <div
                key={hr.id}
                className="p-3 rounded-2xl bg-slate-950/60 border border-slate-800 flex items-center justify-between hover:border-purple-500/30 transition"
              >
                <div className="space-y-0.5">
                  <div className="text-xs font-bold text-white flex items-center space-x-2">
                    <span>{hr.full_name}</span>
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 font-semibold border border-purple-500/30 capitalize">
                      {hr.role === "hr_manager" ? "HR Manager" : "HR Executive"}
                    </span>
                  </div>
                  <div className="text-[11px] text-slate-400 font-mono">{hr.email}</div>
                </div>
            ...
            {pendingHRInvites.map((inv) => (
              <div
                key={inv.id}
                className="p-3 rounded-2xl bg-amber-500/5 border border-amber-500/20 flex items-center justify-between transition"
              >
                <div className="space-y-0.5">
                ...
                <div className="text-right">
                  <span className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-300 text-[10px] font-semibold border border-amber-500/30">
      ...
      <div className="pt-2">
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/30 text-xs font-semibold transition flex items-center justify-center space-x-2"
        >
